...
fit.linear <- (co2_emission ~ euro_standard + transmission_type + poly(engine_capacity, 4) +
...
x = model.matrix(fit.linear, data_complete_1)[,-1] #[-1] means no intercept

y = data_complete_1$co2_emission
...
legend("bottomright", lwd = 1, col = 1:9, legend = colnames(x), cex = .5)
